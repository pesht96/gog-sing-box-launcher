...
Current stable baseline: `v2.39-sb1137`
...
update_server/public/
  ...
  stable/
    ...
    packages/
      .gitkeep
      gog-update-v2.39-sb1137.tar.gz
      gog-update-v2.39-sb1137.tar.gz.sha256
    releases/
      v2.39-sb1137/
        notes.txt
  xray/
...
Current stable release note: this update preserves each router's saved
 subscription, selected nodes, manual domains, and settings. `v2.39-sb1137`
fixes weak-router endpoint DNS failures by using the router system resolver for
sing-box `local-dns` instead of one hard-coded public UDP resolver. The bounded
watchdog recovery remains enabled, while the existing IP-literal DoH
`proxy-dns` path and selected-node persistence are preserved. It also prevents
the router's own dnsmasq upstream queries from being captured by the TUN DNS
hijack, avoiding the `local-dns -> dnsmasq -> TUN` loop and the 150-query
saturation that previously stopped all proxied services on weak routers.
...
```text
https://gog-sing-box-launcher.pages.dev/stable/packages/gog-update-v2.39-sb1137.tar.gz
```

Current stable notes:

```text
https://gog-sing-box-launcher.pages.dev/stable/releases/v2.39-sb1137/notes.txt
```
