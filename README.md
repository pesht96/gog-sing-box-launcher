...
Current stable baseline: `v2.33-sb1137`
...
update_server/public/
  ...
  stable/
    ...
    packages/
      .gitkeep
      gog-update-v2.33-sb1137.tar.gz
      gog-update-v2.33-sb1137.tar.gz.sha256
    releases/
      v2.33-sb1137/
        notes.txt
  xray/
...
subscription, selected nodes, manual domains, and settings. `v2.33-sb1137`
keeps Discord app/login/invite/CDN/media routed through the VPN path, while
adding a narrow Discord voice guard: `discord.media` UDP is routed through
`AUTO-BALANCE` before the global UDP/443 reject. This is not a broad UDP
catch-all and does not make Discord direct-only. The package is published as
...
```text
https://gog-sing-box-launcher.pages.dev/stable/packages/gog-update-v2.33-sb1137.tar.gz
```

Current stable notes:

```text
https://gog-sing-box-launcher.pages.dev/stable/releases/v2.33-sb1137/notes.txt
```
